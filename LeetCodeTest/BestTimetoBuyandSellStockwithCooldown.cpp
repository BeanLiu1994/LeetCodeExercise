...
int BestTimetoBuyandSellStockwithCooldown::maxProfit_dp(vector<int>& prices, int begin, int end)
{
	...
	{
		//dp check
		if (dp_table[begin][end] != -1)
		{
			return dp_table[begin][end];
		}

		//divide
		int max_profit = 0;
		for (vector<int>::iterator divide_point = prices.begin() + 2; divide_point < prices.end() - 2; ++divide_point)
		{
			int middle = divide_point - prices.begin() + begin;
			vector<int> part1{ prices.begin(), divide_point }, part2{ divide_point + 1, prices.end() };
			int temp_profit = maxProfit_dp(part1, begin, middle - 1) + maxProfit_dp(part2, middle + 1, end);
			if (max_profit < temp_profit)
				max_profit = temp_profit;
		}

		//conquer
		int max_profit_nosep = 0;
		for (vector<int>::iterator divide_point = prices.begin(); divide_point < prices.end() - 1; ++divide_point)
		{
			for (vector<int>::iterator divide_point_r = divide_point + 1; divide_point_r < prices.end(); ++divide_point_r)
			{
				int temp_profit = *divide_point_r - *divide_point;
				if (max_profit_nosep < temp_profit)
					max_profit_nosep = temp_profit;
			}
		}
		if (max_profit < max_profit_nosep)
			max_profit = max_profit_nosep;
		dp_table[begin][end] = max_profit;
		return max_profit;
	}
}